...
def build_flutter_deb(version, features):
    ...
    system2('mkdir -p tmpdeb/etc/xconnect/')
    system2('mkdir -p tmpdeb/etc/pam.d/')
    system2('mkdir -p tmpdeb/usr/share/xconnect/files/systemd/')
    ...
    system2(
        'cp ../res/xconnect.service tmpdeb/usr/share/xconnect/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/xconnect.png')
    system2(
        'cp ../res/scalable.svg tmpdeb/usr/share/icons/hicolor/scalable/apps/xconnect.svg')
    system2(
    ...
    os.rename('rustdesk.deb', '../xconnect-%s.deb' % version)
    os.chdir("..")
...
def main():
    ...
    if windows:
        ...
        system2('mv target/release/rustdesk.exe target/release/XConnect.exe')
        pa = os.environ.get('P')
        if pa:
            # https://certera.com/kb/tutorial-guide-for-safenet-authentication-client-for-code-signing/
            system2(
                f'signtool sign /a /v /p {pa} /debug /f .\\cert.pfx /t http://timestamp.digicert.com  '
                'target\\release\\XConnect.exe')
        else:
            print('Not signed')
        system2(
            f'cp -rf target/release/XConnect.exe {res_dir}')
        os.chdir('libs/portable')
        system2('pip3 install -r requirements.txt')
        system2(
            f'python3 ./generate.py -f ../../{res_dir} -o . -e ../../{res_dir}/xconnect-{version}-win7-install.exe')
        system2('mv ../../{res_dir}/xconnect-{version}-win7-install.exe ../..')
        ...
        system2('mv rustdesk-%s-0-x86_64.pkg.tar.zst xconnect-%s-manjaro-arch.pkg.tar.zst' % (
            version, version))
        ...
        system2(
            'mv $HOME/rpmbuild/RPMS/x86_64/xconnect-%s-0.x86_64.rpm ./xconnect-%s-fedora28-centos8.rpm' % (
                version, version))
        ...
        system2(
            'mv $HOME/rpmbuild/RPMS/x86_64/xconnect-%s-0.x86_64.rpm ./xconnect-%s-suse.rpm' % (
                version, version))
        # yum localinstall rustdesk.rpm
    else:
        if flutter:
            ...
            if osx:
                system2(
                    'strip target/release/bundle/osx/XConnect.app/Contents/MacOS/rustdesk')
                system2(
                    'cp libsciter.dylib target/release/bundle/osx/XConnect.app/Contents/MacOS/')
    ...
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/XConnect.app/Contents/MacOS/*
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/XConnect.app
    '''.format(pa))
                system2(
                    'create-dmg "XConnect %s.dmg" "target/release/bundle/osx/XConnect.app"' % version)
                os.rename('XConnect %s.dmg' %
                          version, 'xconnect-%s.dmg' % version)
                if pa:
                    system2('''
    ...
    #rcodesign sign --p12-file ~/.p12/rustdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./xconnect-{1}.dmg
    codesign -s "Developer ID Application: {0}" --force --options runtime ./xconnect-{1}.dmg
    ...
    rcodesign notary-submit --api-key-path ../.p12/api-key.json  --staple xconnect-{1}.dmg
    # verify:  spctl -a -t exec -v /Applications/XConnect.app
    '''.format(pa, version))
                else:
                    print('Not signed')
            else:
                ...
                system2('cp libsciter-gtk.so tmpdeb/usr/share/xconnect/')
                md5_file_folder("tmpdeb/")
